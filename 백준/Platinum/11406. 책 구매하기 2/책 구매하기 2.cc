...
int main() {
    ...
    for (int i = 1; i <= m; ++i) {
        cin >> a;
        graph[s].push_back(i + 100);
        graph[i + 100].push_back(s);
        maximum_flow[s][i + 100] = a;
    }
    for (int i = 1; i <= m; ++i) {
        for (int j = 1; j <= n; ++j) {
            cin >> a;
            graph[i + 100].push_back(j);
            graph[j].push_back(i + 100);
            maximum_flow[i + 100][j] = a;
        }
    }

    cout << find_max_flow(s, t);
